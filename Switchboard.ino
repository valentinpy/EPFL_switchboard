...
Copyright 2016-2018 Samuel Rosset
...
#define Ver 8 //defines the current firmware version. For ease of transfer by I2C, the slave firmware versions are numbered with consecutive integers. 
...
#define Kill_pin A1 //pin on which the kill switch is connected (used as digital input)
...
void setup() 
{
  ...
  if (SwSrc>2) //in case value is not valid, forces internal control 
    SwSrc=0;
  SwMode=EEPROM.read(EE_SwMode); //reads the switching mode from EEPROM (will only be used if switching source is 0 (onboard))
  if (SwMode>3) //in case of invalid value, forces internal timer (this can happen if EEPROM has never been written)
    SwMode=0;
  VMode=EEPROM.read(EE_VMode); // reads the voltage control mode from EEPROM
  if (VMode>2) //in case of invalid value, foces regulated voltage control
    VMode=0;
  ...
  if (SwSrc==0 && (SwMode==1 || SwMode==3)) //if onboard standalone mode and (DC mode or waveform mode), otptocouplers are turned on
  {
  ...
  if (VMode==0) //regulated internal control
  ...
  if (SwSrc==0 && SwMode>=2) //if onboard control and in switching mode or waveform mode, the timer is switched on (SwMode = 2 or 3)
    TIMSK1 = (1 << OCIE1A);
  else 
    TIMSK1=0;
...
void loop()
{
  ...
  if (V_ADC<5) //if raw reading is smaller than 5 (so roughly 0.5% of full scale)
    Vnow=0; //set output to 0 (to avoid errors due to C0 that can turn on HV LED when not needed
  
  if(Wave_meas_flag && SwMode==3) //if it is time to save the actual output voltage in the output array (we do this only in Wavefrom mode (SwMode 3)).
  {
  ...
  if(digitalRead(Kill_pin)&& (!PowerJack||digitalRead(JackMonitor_pin))) //if HV switch on (and the power jack is plugged in if it is being monitored), we regulate the voltage. Else we shut regulation down to avoid voltage spikes. 
  {
    input=(double)Vnow;
    if (SwMode==3 || (SwMode==0 && SwMode_save==3)) //waveform mode or off when previous mode was waveform. The last part is for waveform in limited cycles mode. At the end of a cycle, you want to keep the last voltage value, to avoid transient effects when cycle restart. 
    ...
    if(VMode==0) //internal voltage control (regulated)
    {
      ...
      if (debug)
      {
        ...
        Serial.print(", ");
        Serial.println(HVPS_PID.GetKd(),4);
      }
    }
    else if (VMode==2) //if internal control but open loop
    {
      if (SwMode!=3) //all mode except waveform
        PWM_output=V_to_PWM(Vset);
      else  //waveform mode
        PWM_output=V_to_PWM(Vset_arb);
        
      if(V_ADC==1023) //if the ADC saturates (this can happen on EMCO with lower voltage rating (2kV and down) where 100% of Vout is obtained for PWM signal much lower than 100%)
      {
        do
        {
      ...
      if (debug)
      {
        ...
        Serial.print(Vnow);
        Serial.print(", ");
        if (SwMode==3) //if waveform mode
          Serial.print(Vset_arb);
        else
          Serial.print(Vset);
        Serial.print(", ");
  ...
  if ((Vnow>40) && SwState==true) //set a threshold at 40 V below which the output is considered safe
