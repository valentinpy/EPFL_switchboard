  ...
  while (Serial.available())
   {
	...
	else
	{
	   ...
	   serial_buffer[serial_buffer_i] = '\0'; // Keep the array NULL-terminated
        ...
        if(serial_buffer_i>=19) //reset string if buffer full
          serial_buffer_i=0;
   }  
}
...
void serial_handler()
{
  if (serial_EOS==true) //if a complete command has been read on the serial port
  {
    char tmp[20];
    serial_EOS=false; //reset EOS value
    serial_buffer_i=0; //reset buffer pointer
    
    if (strncmp(serial_buffer,"QJack",5) == 0)//Checks whether Power Jack is connected. 
      Serial.println(digitalRead(JackMonitor_pin));
      
    else if (strncmp(serial_buffer,"SPowJack",8) == 0) //set power Jack mode. 1: Power is provided through power Jack 0: power is provided through 10-pin headers (Multi-channel or touchscreen + battery)
    {
    ...
    else if (strncmp(serial_buffer,"QPowJack",8) == 0) //Query power Jack mode. 1: Power is provided through power Jack 0: power is provided through 10-pin headers (Multi-channel or touchscreen + battery)
      Serial.println(PowerJack);
    else if (strncmp(serial_buffer,"SI2C",4) == 0) //Set I2C address of the board (need to reset board)
    {
    ...
    else if (strncmp(serial_buffer,"QI2C",4) == 0) //Read I2C address of the board
      Serial.println(I2C_addr);
      
    else if (strncmp(serial_buffer,"SVmax",5) == 0) //Set Max Voltage of the board
    {
    ...
    else if (strncmp(serial_buffer,"QVmax",5) == 0) //Query Max voltage of the board
      Serial.println(Vmax);

    else if (strncmp(serial_buffer,"Conf",4) == 0) //initial configuration of the board to default values (only if board is one of the std ratings: 5000V, 2000V, 3000V, 1200V, 500V). Required parameter: Vmax (board V rating)
    {
      double Ki,Kd,Kp;
      strcpy(tmp,&serial_buffer[5]);  //skip the "SVmax" command
      Vmax=word(atoi(tmp));
    ...
    else if (strncmp(serial_buffer,"SVset",5) == 0) //Set voltage setpoint
    {
      strcpy(tmp,&serial_buffer[5]);  //skip the "SVset" command
      Vset=constrain(word(atoi(tmp)),0,Vmax);
      Serial.println(Vset);
    }
    else if (strncmp(serial_buffer,"QVset",5) == 0) //Query voltage setpoint
      Serial.println(Vset);
      
    else if (strncmp(serial_buffer,"QVnow",5) == 0) //Query voltage setpoint
      Serial.println(Vnow);
      
    else if (strncmp(serial_buffer,"SPWM",4) == 0) //Set voltage setpoint as PWM signal (for open loop mainly)
    {
    ...
    else if (strncmp(serial_buffer,"QPWM",4) == 0) //Query voltage setpoint as PWM signal
      Serial.println(V_to_PWM(Vset));
    
    else if (strncmp(serial_buffer,"SSwSrc",6) == 0) // Set the Switching source
    {
    ...
    else if (strncmp(serial_buffer,"QSwSrc",6) == 0) //Query switching source
      Serial.println(SwSrc); //0: onboard (standalone) 1: external (from Master) 2: push button
      
    else if (strncmp(serial_buffer,"SSwMode",7) == 0) // Set the Switching mode
    {
    ...
    else if (strncmp(serial_buffer,"SName",5) == 0) //Query the cycle number, and the total number of cycle should be made (0=illimited)
    {
    ...
    else if (strncmp(serial_buffer,"QName",5) == 0) //Query the cycle number, and the total number of cycle should be made (0=illimited)
    {
      Serial.println(Name);
    }
